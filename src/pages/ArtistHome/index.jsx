...
import { AddCardItem, PenddingCardItem } from "../Myprofile/CardItem";

const ArtistHomeStyled = styled.div`
	.top_bar {
		position: relative;
		.bg_wrapper {
			width: 100%;
...
export function ArtistHome() {
	...
	const NavList = [
		{
			title: wrapperIntl("ArtistHome.fineArts"),
			route: "FineArts",
			channelRequestParam: "Fine Arts",
		},
		{
			title: wrapperIntl("ArtistHome.sports"),
			route: "Sports",
			channelRequestParam: "Sports",
		},
		{
			title: wrapperIntl("ArtistHome.comics"),
			route: "Comics",
	...
	const [channelRequestParam, setChannelRequestParam] = useState(
		channel === NavList[0].route
			? NavList[0].channelRequestParam
			: channel === NavList[1].route
			? NavList[1].channelRequestParam
			: NavList[2].channelRequestParam
	);
	const [categoryRequestParam, setCategoryRequestParam] = useState(
		wrapperIntl("Category.All")
	);
		...
		const tradeAuctions = tradeData.tradeAuctions.filter((item) => {
			return (
				...
				String(item.token0).toLowerCase() ===
					String(brandInfo.contractaddress).toLowerCase()
			);
		});
	...
	const changeDataByCannel = async (tradeArr) => {
		...
		sign_Axios
			.post(Controller.items.getitemsbyfilter, {
				ids: ids,
				cts: cts,
					...
					const list = tradeArr
						.map((poolInfo) => {
							const item = res.data.data.find(
								(item) => poolInfo.tokenId === item.id
							);
							// console.log(poolInfo)
							return {
								...item,
								poolType: poolInfo.amount_total0
									? AUCTION_TYPE.FixedSwap
									: AUCTION_TYPE.EnglishAuction,
	...
	useEffect(() => {
		sign_Axios
			.post(Controller.brands.getbrandbyid, {
				id: Number(id),
			})
			.then((res) => {
				if (res.status === 200 && res.data.code === 1) {
	...
	const renderListByChannel = (channel) => {
		...
		return (
			<ul
				className={`list_wrapper ${channel}`}
		...
		<ArtistHomeStyled>
			<div className="top_bar">
				<div
					...
					style={
						brandInfo?.bandimgurl
							? {
									backgroundSize: "100%!important",
									background: `url(${brandInfo.bandimgurl}) center center no-repeat`,
							  }
							: {}
					}
				>
					{brandInfo?.owneraddress &&
						String(brandInfo.owneraddress).toLowerCase() ===
							String(account).toLowerCase() && (
							<button
								onClick={() => setOpenUpdateTopBarImg(true)}
							>
								<img src={edit_white} alt="" />
								<p>{wrapperIntl("ArtistHome.Change")}</p>
							</button>
						)}
				</div>
				<div className="userinfo">
					<img
						src={/* brandInfo.bandimgurl */ brandInfo.imgurl}
						alt=""
					/>
          ...
          return <li key={item.name} className={channel === item.name ? 'active' : ''} onClick={() => {
            history.push(`/ArtistHome/${id}/${standard}/${item.name}`)
          }}>
            <p className="flex flex-center-y"><img src={
              item.name === NFT_CATEGORY.FineArts ? icon_arts :
					...
					{NavList.map((nav) => {
						return (
							<li
								key={nav.title}
								className={
									channel === nav.route ? "active" : ""
								}
								onClick={() => {
									setChannelRequestParam(
										nav.channelRequestParam
									);
									history.push(
										`/ArtistHome/${id}/${standard}/${nav.route}`
									);
							...
							>
								<p className="flex flex-center-y">
									<img
										src={
											nav.title === NFT_CATEGORY.FineArts
												? icon_arts
												: nav.title ===
												  NFT_CATEGORY.Sports
												? icon_sport
												: nav.title ===
												  NFT_CATEGORY.ComicBooks
												? icon_comics
												: ""
										}
										alt=""
				...
				<div className="filterBox">
					<Search
						placeholder={wrapperIntl("ArtistHome.placeholder")}
					/>
					...
					<div style={{ opacity: 0 }}>
						<PullRadioBox
							prefix={"Sort by:"}
							width={"204px"}
							options={[{ value: "New" }, { value: "Popular" }]}
							defaultValue="New"
							onChange={(item) => {
								// console.log(item)
							}}
				...
				{active && !loading && itemList.length > 0 ? (
					<ul className="list">
						{/* <li>
              <AddCardItem />
            </li> */}
								...
								<li key={index}>
									{item.isPendding ? (
										...
										<CardItem
											...
											itemname={
												item.itemname === ""
													? "Unname (External import)"
													: item.itemname
											}
											user={item.ownername}
											status={
												parseInt(item.poolId) >= 0 &&
												wrapperIntl("Listed")
											}
											poolType={item.poolType}
				...
				{active && loading && (
					<SkeletonNFTCards n={4}></SkeletonNFTCards>
				)}

				{/* <PagingControls /> */}
			</MarketplaceStyled>
			<UpdateTopBarImg
				open={openUpdateTopBarImg}
				setOpen={setOpenUpdateTopBarImg}
				run={run}
			/>
		</ArtistHomeStyled>
	);
