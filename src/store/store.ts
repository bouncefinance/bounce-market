...
import { abortRequests, getQuery, handleRequests } from '@redux-requests/core';
...
const axiosMainApiInit = axios.create({
  baseURL: process.env.REACT_APP_API_BASE,
});
axiosMainApiInit.interceptors.response.use(response => {
  // token invalid
...
const { requestsReducer, requestsMiddleware } = handleRequests({
  driver: {
    ...
    mainApiSmartchain: createAxiosDriver(axiosMainApiInit),
