import { ChainType } from 'modules/account/hooks/chainConfig';

// interface IApiSearchDropsItem

export const mockActive = [
  {
    accountaddress: '0xc6a34b2bf59baf984884a0cf4c84ed1541e710d7',
    avatar: '',
    bgcolor: '',
    blindcoverimgurl:
      'https://ap1-cfs3-media-bounce.bounce.finance/2439e38376c040f2c6cdf6523f299655-1637315808.jpg',
    collection: '0x315eb39921F2499Ef698DE3DDEEc63219C02541A',
    coverimgurl:
      'https://ap1-cfs3-eth-fib.fangible.com/d51ac520b1f0f73c5614780864c00578-1639469218.png',
    created_at: '2021-12-06T09:53:37Z',
    description:
      'The Boxing Bullies by Jake Paul NFT Collection commemorates Jake Paul’s historic second fight against Tyron Woodley, with exclusive releases from Sneaky Vampire Syndicate and Wilder World.',
    display: 1,
    dropdate: 1638784476,
    droptype: 3,
    facebook: '',
    id: 11,
    instagram: '',
    nftdescription: '',
    nfts: 108,
    price: '',
    state: 2,
    title: 'Exclusive Boxing Bullies by Jake Paul NFT Collection',
    twitter: '',
    updated_at: '2021-12-09T03:27:39Z',
    username: '0xc6a34b2bf59baF984884A0cf4C84eD1541E710d7',
    videourl: '',
    website: '',
    directLink:
      'https://fangible.com/profile/address/0xc3204884fad1fbc91b52e2fab27b7236f6e27137?id=&tab=sale',
    tarChain: 4 as ChainType,
  },
  {
    accountaddress: '0x8031957cEecEe4Dbd63935def5745940D6d75Bc6',
    avatar: '',
    bgcolor: '',
    blindcoverimgurl: '',
    collection: '',
    coverimgurl:
      'https://ap1-cfs3-eth-fib.fangible.com/592631e73e6e61013b5bba49827344c3-1638772183.png',
    created_at: '2021-12-06T06:30:00Z',
    description:
      'Jake Paul’s Boxing Bullies joins forces with Space Runners Metaverse NFT Fashion brand to fight against bullying. The auction features 2 unique sneaker NFTs,where all proceeds will be donated to fight bullying.',
    display: 1,
    dropdate: 1638772243,
    droptype: 1,
    facebook: '',
    id: 5,
    instagram: '',
    nftdescription: '',
    nfts: 0,
    price: '',
    state: 2,
    title: 'Exclusive Boxing Bullies x Space Runners NFT Collection',
    twitter: '',
    updated_at: '2021-12-06T06:30:46Z',
    username: '0x8031957cEecEe4Dbd63935def5745940D6d75Bc6',
    videourl: '',
    website: '',
    directLink:
      'https://solana.fangible.com/profile/address/5nmTGuPPcVw5jy5mNXpnqLCR3fJ7cnBUt28oKFgPpHfE?id=&tab=sale',
    tarChain: 1111 as ChainType, // 1111 是 Solana
    // comingTime: 'Coming Early 2022',
  },
];
